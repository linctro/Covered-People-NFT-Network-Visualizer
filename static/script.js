...
const CONFIG = {
    colors: {
        ...
        ritoBeer: '#57d9d0',
        kabaYoko: '#fceb15',
        kabaYokoStroke: '#0f6db0',
        nafudaColors: ['#c12f2a', '#038fda', '#6fb72a', '#f9bd00', '#91258d'],
        nafudaGlow: 'rgba(255, 255, 180, 0.6)',
    ...
    contracts: {
        ...
        ritoBeer: "0x5d94faa2d2bf6a7c72a8d4925b931afd470e894e".toLowerCase(),
        kabaYoko: "0xbe8ec65fe21314622a3e7ec4932c9236e83a6f47".toLowerCase(),
        nafuda: "0x7a91d5c6a2d55b888ddcddc44e3e113ef49852b8".toLowerCase()
    },
    // Collection type to color mapping
    typeColorMap: {
        'Genesis': 'rgba(255, 215, 0, 0.7)',
        'Generative': 'rgba(0, 255, 127, 0.5)',
        'RitoBeer': '#57d9d0',
        'KabaYoko': '#fceb15',
        'Nafuda': '#f9bd00' // fallback color for simple operations
    }
};
        ...
        if (nodes.length === 0) {
            ...
            const processChunk = () => {
                ...
                for (let i = count; i < end; i++) {
                    const item = nodes[i];
                    const typeLabel = item._custom_type || 'Generative';
                    const color = CONFIG.typeColorMap[typeLabel] || CONFIG.colors.generative;
...
function addNodeIncrementally(tx, typeColor, typeLabel) {
    ...
    tokenHistories[tokenId].forEach((event, idx) => {
        ...
        const nodeRadius = CONFIG.radii[typeLabel.charAt(0).toLowerCase() + typeLabel.slice(1)] || CONFIG.radii.node;
...
function render() {
    ...
    for (const node of state.nodes) {
        ...
        if (node.id === 'issuer') {
            ...
            // Standard Node Rendering - shape depends on nftType
            const nftType = node.nftType || 'Generative';

            if (nftType === 'RitoBeer') {
                // ■ Square shape
                const size = effectiveRadius * 1.7;
                ctx.fillStyle = (isHovered || isSelected) ? '#ffffff' : node.color;
                if (isSelected || isHovered) { ctx.shadowBlur = 15; ctx.shadowColor = node.color; }
                ctx.fillRect(node.x - size / 2, node.y - size / 2, size, size);
                ctx.shadowBlur = 0;
                // For hit detection, still use beginPath area
                ctx.beginPath();
                ctx.rect(node.x - size / 2, node.y - size / 2, size, size);

            } else if (nftType === 'KabaYoko') {
                // ● Circle with colored stroke
                ctx.beginPath();
                ctx.arc(node.x, node.y, effectiveRadius, 0, Math.PI * 2);
                ctx.fillStyle = (isHovered || isSelected) ? '#ffffff' : CONFIG.colors.kabaYoko;
                if (isSelected || isHovered) { ctx.shadowBlur = 15; ctx.shadowColor = node.color; }
                ctx.fill();
                ctx.shadowBlur = 0;
                ctx.lineWidth = 1.5 / state.transform.scale;
                ctx.strokeStyle = CONFIG.colors.kabaYokoStroke;
                ctx.stroke();

            } else if (nftType === 'Nafuda') {
                // ⬠ Pentagon with conic gradient + glow
                const r = effectiveRadius * 1.5;
                ctx.beginPath();
                for (let i = 0; i < 5; i++) {
                ...
                if (ctx.createConicGradient) {
                    const grad = ctx.createConicGradient(0, node.x, node.y);
                    const colors = CONFIG.colors.nafudaColors;
                    colors.forEach((c, i) => grad.addColorStop(i / colors.length, c));
                    grad.addColorStop(1, colors[0]);
                    ctx.fillStyle = (isHovered || isSelected) ? '#ffffff' : grad;
                } else {
                    ctx.fillStyle = (isHovered || isSelected) ? '#ffffff' : CONFIG.colors.nafudaColors[0];
                }
                ctx.fill();

                // Glow border
                ctx.shadowBlur = (isSelected || isHovered) ? 20 : 8;
                ctx.shadowColor = CONFIG.colors.nafudaGlow;
                ctx.lineWidth = 1.5 / state.transform.scale;
                ctx.strokeStyle = CONFIG.colors.nafudaGlow;
                ctx.stroke();
                ctx.shadowBlur = 0;

            } else {
                // Default: circle (Genesis, Generative)
                ctx.beginPath();
                ctx.arc(node.x, node.y, effectiveRadius, 0, Math.PI * 2);
                ctx.fillStyle = (isHovered || isSelected) ? '#ffffff' : node.color;
                if (isSelected || isHovered) { ctx.shadowBlur = 15; ctx.shadowColor = node.color; }
...
function setupInteraction() {
    ...
    const cbRitoBeer = document.getElementById('cb-ritobeer');
    const cbKabaYoko = document.getElementById('cb-kabayoko');
    const cbNafuda = document.getElementById('cb-nafuda');

    if (cbRitoBeer) cbRitoBeer.onchange = (e) => { state.visibility.RitoBeer = e.target.checked; };
    if (cbKabaYoko) cbKabaYoko.onchange = (e) => { state.visibility.KabaYoko = e.target.checked; };
    if (cbNafuda) cbNafuda.onchange = (e) => { state.visibility.Nafuda = e.target.checked; };
